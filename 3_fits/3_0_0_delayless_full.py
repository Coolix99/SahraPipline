...
def posterior_diagnostics(fit, data_dict):
    ...
    output_file("3_fits/fig_3_0_0/ppc_Reg.html")
    save(predictive_regression(
        f_ppc_reg,
        samples_x=np.array(data_dict["t_ppc"]),
    ...
    results_df.to_csv(os.path.join(fit_results_path, "area_sampled_parameter_results_delayless_full.csv"), index=False)
