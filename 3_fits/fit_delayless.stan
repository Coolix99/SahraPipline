functions {
  ...
  real g_A(real A, real beta_, real A_end, real A_cut) {
    if (A < A_cut)
  ...
  vector ode_system(real t, vector y, array[] real params) {
    ...
    vector[1] dydt;
    dydt[1] = A * g_A(A, beta_, A_end, A_cut);
    return dydt;
  }

  array[] real A_theor(array[] real t, real A_0, real beta_, real A_end, real A_cut) {
    vector[1] y0 = [A_0]';
    array[3] real params = {beta_, A_end, A_cut};
  ...
  real A_theor(real t, real A_0, real beta_, real A_end, real A_cut) {
    array[1] real t_arr = {t};
    return A_theor(t_arr, A_0, beta_, A_end, A_cut)[1];
  }
}
...
model {
  ...
  // Likelihood
  A_Dev ~ normal(A_theor(t_Dev, A_0_Dev, beta_, A_end, A_cut), sigma);
  A_Reg ~ normal(A_theor(t_Reg, A_0_Reg, beta_, A_end, A_cut), sigma);
}

generated quantities {
  array[N_ppc] real A_Dev_ppc = normal_rng(A_theor(t_ppc, A_0_Dev, beta_, A_end, A_cut), sigma);
  array[N_ppc] real A_Reg_ppc = normal_rng(A_theor(t_ppc, A_0_Reg, beta_, A_end, A_cut), sigma);
}
